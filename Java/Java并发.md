...
* 既可以保证可见性，又能够保证原子性。
    ...
    * 原子性表现在：要么不执行，要么执行到底。
* synchronized不具有继承性。
* synchronized是一种高开销的操作，因此应该尽量减少同步的粒度。
...
#### 自旋锁
如果持有锁的线程能在很短时间内释放锁资源，那么尝试获取锁的线程不会立即阻塞，而是采用循环的方式去尝试获取锁，这样的好处是减少线程上下文（内核态和用户态）切换的消耗，缺点是循环会消耗CPU。
