...
* GET和POST本质上都是TCP链接，但是由于HTTP的规定和浏览器和服务器的限制，导致他们在应用过程中体现出一些不同。最关键的不同是：GET产生一个TCP数据包，而POST产生两个TCP数据包。过程区别如下：GET请求会把请求头和请求体一起发送，服务器响应200；而POST会先发送请求头，服务器响应100之后，浏览器再发送请求体，服务器收到后再响应200。
...
4. 如果这些DNS服务器都查不到，说明域名没有做好解析设置；如果查找了，就把对应的IP地址发回LDNS，再有LDNS发回给客户端。
...
### 原理
负载均衡（Load Balance）是将负载（工作任务、访问请求）进行平衡，分摊到多个操作单元（节点）上进行执行。
